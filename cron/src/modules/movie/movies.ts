...
export const movieService = async (): Promise<void> => {
  let page = 1,
    total_pages = 1;

  while (page <= total_pages) {
    console.time(`Fetch and store time for page ${page}`);
    try {
      const { data }: { data: MovieApiResponse } = await fetchWithRetry(
        endpoints.discover.movie(page)
      );
      ...
      await handleError(error, () => movieService());
    }
    console.timeEnd(`Fetch and store time for page ${page}`);
    page++;
  }
};
